<script lang="ts">
    ...
    function filterMovies() {
        if (searchQuery) {
            ...
            console.log('Filtered Movies:', filteredMovies); // Debugging statement
        } else {
    ...
    <div class="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto" on:click={handleOutsideClick}>
        <div class="modal-content bg-surface-800 text-surface-50 rounded-lg w-1/2 relative" role="dialog" on:click|stopPropagation>
            ...
            <div class="relative w-full h-4/5">
                ...
                <iframe bind:this={iframeElement} title={`Trailer for ${selectedMovie.title}`} src={`https://www.youtube.com/embed/${selectedMovie.youtube_trailer_url}?autoplay=1&controls=0&mute=${muted ? 1 : 0}&loop=1&rel=0`} class="w-full h-[400px] object-fit rounded-t-lg pointer-events-none"></iframe>
