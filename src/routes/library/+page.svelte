            ...
            {#each bookmarkedMoviesList as movie, index}
                <div class="card w-full h-[380px] rounded-lg transform hover:scale-[115%] transition-transform duration-300 relative hover:z-10"
                    ...
                    {#if hoverStates[index]}
                        <iframe src={`https://www.youtube.com/embed/${movie.youtube_trailer_url}?autoplay=1&controls=0&mute=1&loop=1`} class="w-full h-3/4 object-cover rounded-t-lg pointer-events-none" title="Trailer for {movie.title}"></iframe>
                    {:else}
                    ...
                    <div class="flex justify-between items-center mt-2 pl-4 pr-4">
                        ...
                        <button type="button" class="btn-icon variant" on:click|stopPropagation={(event) => toggleBookmark(event, movie.movie_id)}>
