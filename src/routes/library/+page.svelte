    ...
    <div class="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto" on:click={handleOutsideClick}>
        <div class="modal-content bg-surface-800 text-surface-50 rounded-lg w-1/2 relative" role="dialog" on:click|stopPropagation>
            ...
            <div class="relative w-full h-4/5">
                <iframe bind:this={iframeElement} title={`Trailer for ${selectedMovie.title}`} src={`https://www.youtube.com/embed/${selectedMovie.youtube_trailer_url}?autoplay=1&controls=0&mute=${muted ? 1 : 0}&loop=1`} class="w-full h-[400px] object-fit rounded-t-lg pointer-events-none"></iframe>
...
<AppShell>
    ...
    <section class="flex w-full h-full">
        ...
        <AppRail class="flex flex-col items-center justify-center pb-40 fixed">
            ...
            <AppRailAnchor selected={$page.url.pathname === '/library'} name="library" value={2} title="Library" href="/library">
                <svelte:fragment slot="lead">
                    <Library class="w-20"/>
                </svelte:fragment>
            </AppRailAnchor>
            ...
            {#each bookmarkedMoviesList as movie, index}
                <a class="card w-full h-[380px] rounded-lg transform hover:scale-[115%] transition-transform duration-300 relative hover:z-10"
                    ...
                    {#if hoverStates[index]}
                        <iframe src={`https://www.youtube.com/embed/${movie.youtube_trailer_url}?autoplay=1&controls=0&mute=1&loop=1`} class="w-full h-3/4 object-cover rounded-t-lg pointer-events-none"></iframe>
                    {:else}
                        <img src={movie.poster} alt={movie.title} class="w-full h-3/4 object-cover rounded-t-lg">
                    {/if}
