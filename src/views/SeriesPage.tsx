...
export function SeriesPage() {
  ...
  const { heroItem, carousels } = useMemo(() => {
    if (seriesItems.length === 0) return { heroItem: null as CarouselItem | null, carousels: [] as { title: string; items: CarouselItem[] }[] };
    ...
    const myListItems = seriesItems.filter((item) => isInMyList(item.id));
  ...
  }, [seriesItems, libraryCarousels, detailsMap, progressByItemId, isInMyList]);
  ...
  const moreLikeThisItems = useMemo(() => {
    ...
    return seriesItems.filter((item) => item.id !== selectedItem.id).slice(0, 6);
  }, [seriesItems, selectedItem?.id]);
